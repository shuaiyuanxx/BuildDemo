...
int main()
{
    ...
    printf("[WORKER] Uppercase worker listening on port %d...\n", PORT);
    fflush(stdout);
    ...
    while (true)
    {
        ...
        printf("[WORKER] Client connected: %s:%d\n", inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
        fflush(stdout);
        ...
        while (true)
        {
            ...
            if (n <= 0) break;
            buf[n] = '\0';
            printf("[WORKER] Received: %s", buf);
            fflush(stdout);

            // Transform to uppercase before sending back
            for (int i = 0; i < n; i++)
                buf[i] = toupper((unsigned char)buf[i]);

            printf("[WORKER] Sending:  %s", buf);
            fflush(stdout);
